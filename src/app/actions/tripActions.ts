...
export async function createTrip(formData: FormData) {
    try {
        ...
        const trip: Trip = {
            ...
            difficulty: (formData.get('difficulty') as Trip['difficulty']) || 'Moderate',
            status: (formData.get('status') as Trip['status']) || 'available',
            ...
            duration: (formData.get('duration') as Trip['duration']) || '2D1N',
            type: (formData.get('type') as Trip['type']) || 'join',
            description: (formData.get('description') as string) || '',
            location: (formData.get('location') as string) || '',
            highlights: safeParse('highlights'),
            itinerary: safeParse('itinerary'),
            whatsIncluded: safeParse('whatsIncluded'),
            notIncluded: safeParse('notIncluded'),
        };
...
export async function updateTrip(originalId: string, formData: FormData) {
    ...
    try {
        ...
        const slugInput = formData.get('slug') as string;
        newId = slugInput ? slugify(slugInput) : originalId;
